...
def test_end_condition_simple():
    ...
    assert dict(tree[root][sent]) == {
        'label': 'sent',
    }
    assert dict(tree[sent][received]) == {
        'label': 'received',
    }
...
def test_recursion():
    ...
    graph = networkx.DiGraph([
        ('s1', 's2', {
            0: {'label': 'sentA / receivedA'}
        }),
        ('s2', 's2', {
            0: {'label': 'sentB / receivedB'},
        }),
    ])
    ...
    assert dict(tree[root][sentA]) == {
        'label': 'sentA',
    }
    assert dict(tree[sentA][receivedA]) == {
        'label': 'receivedA',
    }
    assert dict(tree[receivedA][sentB]) == {
        'label': 'sentB',
    }
    assert dict(tree[sentB][receivedB]) == {
        'label': 'receivedB',
    }
